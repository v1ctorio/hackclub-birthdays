...
pub fn update_member(req: Request, ctx: Context, id: String) -> Response {
  use _req, claims <- web.auth_mw(req, ctx)
  use <- bool.guard(when: claims.hca_id != id, return: { wisp.response(403) })

  let db = context.db_conn(ctx)
  use json <- wisp.require_json(req)
  use member_input <- web.decode_body(json, member.member_input_decoder())

  let member = member.to_member(member_input, claims.hca_id, claims.slack_id)
...
pub fn get_me(req: Request, ctx: Context) -> Response {
  use _req, claims <- web.auth_mw(req, ctx)
  let db = context.db_conn(ctx)
  use member <- web.db_execute(repository.get_member(db, claims.hca_id))
  member
  |> member.member_to_json
  |> json.to_string
  |> wisp.json_body(wisp.ok(), _)
